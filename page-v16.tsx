...
export default function Home() {
  ...
  // Keep refs in sync
  const handleModelChange = (v: string) => { setModel(v); modelRef.current = v }
  const handleStyleChange = (v: string) => { setStyle(v); styleRef.current = v }
  const handleRatioChange = (v: string) => { setRatio(v); ratioRef.current = v }
  ...
  async function generateImages(npcName: string, imagePrompt: string, negativePrompt: string) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      try {
        ...
        if (data.image) {
          ...
          setNpcs(prev => prev.map(n => n.name === npcName ? { ...n, images: [...results] } : n))
        }
      } catch { /* continue */ }
    }
    return results
  ...
  async function generateForNPC(npcName: string) {
    setNpcs(prev => prev.map(n => n.name === npcName ? { ...n, loading: true, images: [] } : n))
    const npc = npcs.find(n => n.name === npcName)
    if (!npc) return
    await generateImages(npc.name, npc.image_prompt, npc.negative_prompt)
    setNpcs(prev => prev.map(n => n.name === npcName ? { ...n, loading: false } : n))
  }
  ...
  async function generateAll() {
    ...
    for (const npc of npcs) {
      if (stopRef.current) break
      setNpcs(prev => prev.map(n => n.name === npc.name ? { ...n, loading: true, images: [] } : n))
      await generateImages(npc.name, npc.image_prompt, npc.negative_prompt)
      setNpcs(prev => prev.map(n => n.name === npc.name ? { ...n, loading: false } : n))
    }
    setGenerating(false)
    stopRef.current = false
  }
  ...
  function toggleSelect(name: string) {
    setNpcs(prev => prev.map(n => n.name === name ? { ...n, selected: !n.selected } : n))
  }
  ...
  function exportToXlsx() {
    ...
    const headers = ['角色名', '描述', '生图Prompt', '图像', '备注']
  ...
  function exportToHtml() {
    ...
    const rows = selected.map(npc => {
      ...
      return `<tr><td class="name-cell">${npc.name}</td><td class="desc-cell">${npc.description || ''}<br/><span class="prompt">${npc.image_prompt || ''}</span></td>${imgCells}</tr>`
    }).join('')
    const html = `<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>NPC 角色表</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,sans-serif;background:#f9f9f7;padding:32px;color:#1a1a1a}h1{font-size:20px;font-weight:600;margin-bottom:4px}.meta{font-size:13px;color:#888;margin-bottom:24px}table{width:100%;border-collapse:collapse;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.08)}thead{background:#f0efeb}th{padding:12px 14px;text-align:left;font-size:12px;font-weight:500;color:#555;border-bottom:1px solid #e8e6e0}td{padding:12px 14px;border-bottom:1px solid #f0efeb;vertical-align:top}tr:last-child td{border-bottom:none}.name-cell{font-weight:600;font-size:14px;min-width:80px;white-space:nowrap}.desc-cell{font-size:12px;line-height:1.6;color:#444;max-width:200px}.prompt{color:#999;font-style:italic;margin-top:4px;display:block}.img-cell{padding:8px;min-width:160px}.img-cell img{width:160px;height:90px;object-fit:cover;border-radius:6px;display:block}.img-cell.empty{color:#ccc;font-size:12px;text-align:center}</style></head><body><h1>NPC 角色表</h1><p class="meta">共 ${selected.length} 个角色 · 生成于 ${new Date().toLocaleDateString('zh-CN')}</p><table><thead><tr><th>角色名</th><th>描述</th><th>图像 1</th><th>图像 2</th><th>图像 3</th><th>图像 4</th></tr></thead><tbody>${rows}</tbody></table></body></html>`
    const blob = new Blob([html], { type: 'text/html;charset=UTF-8' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `npc_${projectName || 'characters'}.html`
    a.click()
    URL.revokeObjectURL(url)
    setExportStatus('✓ HTML 已下载，用浏览器打开查看')
  }
    ...
    <main style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem 1rem', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <select value={style} onChange={e => handleStyleChange(e.target.value)} style={selectStyle}>
            {STYLES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <label style={{ fontSize: 13, color: '#555' }}>比例</label>
          <select value={ratio} onChange={e => handleRatioChange(e.target.value)} style={selectStyle}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{ ...btnSmall, background: selectedCount > 0 ? '#555' : '#ccc', color: '#fff', borderColor: 'transparent' }}
              >
              ...
              >
                <div style={{ background: '#f5f5f5', aspectRatio: ratio === '16:9' ? '16/9' : ratio === '9:16' ? '9/16' : '1/1', position: 'relative', overflow: 'hidden' }}>
                  {npc.images.length > 0 ? (
                    ...
                    <div style={centerFlex}>
                      <button onClick={e => { e.stopPropagation(); generateForNPC(npc.name) }} style={btnPrimary}>生成图像</button>
                    </div>
                  )}
                  {npc.selected && (
                ...
                <div style={{ padding: '10px 12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                    {npc.images.length > 0 && !npc.loading && (
                      <>
                        <button onClick={e => { e.stopPropagation(); generateForNPC(npc.name) }} style={{ ...btnSmall, fontSize: 11 }}>重新生成</button>
